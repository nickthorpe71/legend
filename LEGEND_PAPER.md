...
Legend enforces hard capacity limits for working and episodic memory: 10 L1 entries and 1,024 L2 entries. L3 graph nodes are open-ended because they are compressed semantic representations rather than raw transcript storage; weak or stale nodes still decay below the pruning threshold. L3 edges currently retain an engineering cap of 8,192 pending separate edge-capacity policy review. Pruning, salience renormalization (gentle EMA blend every 10 ticks), and graph weight normalization (ceiling 2.0 every 5 ticks) prevent reinforcement blow-up.
