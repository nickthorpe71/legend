...
use steps::frame::assemble_frame;
...
fn tick_via_daemon(input: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match resp {
        daemon::DaemonResponse::Frame(frame) => {
            // Frame is the entire observable surface
            // (§docs/frame-as-surface.md). Rendering is a pure function
            // over the frame; the CLI is a relay.
            print!("{}", render::render_frame(&frame));
            Ok(())
        }
