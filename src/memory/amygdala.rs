...
const SEED_PROTOTYPES: &[(&str, f32, &str)] = &[
    ...
    ("we lost important data and cannot recover it", -0.9, "negative"),
    ("this keeps failing no matter what I try", -0.6, "negative"),
    ("something died or was destroyed", -0.8, "negative"),
    ("there is a serious security vulnerability", -0.7, "negative"),
    ("the deadline was missed and we failed to deliver", -0.6, "negative"),
    ("this error is catastrophic and unrecoverable", -0.9, "negative"),
    ("the system crashed and corrupted the data", -0.8, "negative"),
    ("we have a critical production outage", -0.8, "negative"),
    ("this is a terrible mistake with serious consequences", -0.7, "negative"),
    ("performance has degraded severely", -0.5, "negative"),
    ("the project was cancelled after months of work", -0.7, "negative"),
    ("trust has been broken and damage was done", -0.7, "negative"),
    ("this situation is hopeless and getting worse", -0.8, "negative"),
    ("a major regression was introduced", -0.6, "negative"),
    ("we are losing users and revenue is dropping", -0.6, "negative"),
    ("the integration is completely incompatible", -0.5, "negative"),
    ("someone made an unauthorized destructive change", -0.7, "negative"),
    ("this vulnerability exposes sensitive user data", -0.8, "negative"),
    ("the entire test suite is failing", -0.6, "negative"),
    ("we discovered a fundamental design flaw", -0.7, "negative"),
    ("resources are exhausted and the system is unresponsive", -0.6, "negative"),
    ("there was a painful loss that cannot be undone", -0.9, "negative"),
    ("this failure affects all downstream systems", -0.7, "negative"),
    ...
    ("all tests are passing and coverage improved", 0.5, "positive"),
    ("the migration completed without any issues", 0.5, "positive"),
    ("performance improved dramatically", 0.6, "positive"),
    ("received great feedback from users", 0.6, "positive"),
    ("the refactor made everything cleaner and simpler", 0.6, "positive"),
    ...
    ("got accepted and recognized for the achievement", 0.8, "positive"),
    ("this collaboration produced amazing results", 0.7, "positive"),
    ("the optimization reduced costs significantly", 0.6, "positive"),
    ("everything came together beautifully", 0.7, "positive"),
    ("the fix resolved the issue permanently", 0.6, "positive"),
    ("we successfully recovered from the incident", 0.6, "positive"),
    ...
    ("there is no time left and the deadline is now", 0.0, "urgency"),
    ("this requires immediate attention", 0.0, "urgency"),
    ("everything else must stop until this is resolved", 0.0, "urgency"),
    ...
    ("this has been escalated to the highest level", 0.0, "urgency"),
    ("immediate rollback is required", 0.0, "urgency"),
    ("production is down and customers are affected", 0.0, "urgency"),
    ...
    ("this is a showstopper that must be fixed now", 0.0, "urgency"),
...
mod tests {
    ...
    fn test_neutral_text() {
        let v =
            compute_emotional_valence_keywords("updated the documentation formatting", &kw());
    ...
    fn test_urgency_amplifies_negative() {
        let without =
            compute_emotional_valence_keywords("there is a bug in the system", &kw());
        let with =
            compute_emotional_valence_keywords("BLOCKER: critical bug in the system", &kw());
        assert!(
            with < without,
    ...
    fn test_prototype_negative_vs_positive_direction() {
        ...
        assert!(v_neg < 0.0, "loss/destruction should be negative, got {}", v_neg);
        assert!(v_pos > 0.0, "breakthrough/success should be positive, got {}", v_pos);
    }
    ...
    fn test_prototype_urgency_preserves_direction() {
        ...
        let emb = embed("critical production outage, everything is down and needs immediate action");
        let v = compute_emotional_valence(&protos, &emb);
        assert!(
            v < 0.0,
