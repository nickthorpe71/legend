...
//! on `ExtractionOutput` exist to preserve `build_relations`'s batch processing
...
/// Everything `run_extractors` contributes to a tick. Every relation-shaped
/// proposal is a [`RelationCandidate`] regardless of source; the
/// `known` vs `novelty` split is preserved only because `build_relations` batch-
...
pub struct NoveltyExtractions {
    /// Phrase + Token chunks from the orthographic chunker.
...
pub fn run_extractors(
    ...
    unconditional_chunks.extend(extract_content_tokens(input_text, hypergraph));
    ...
    let label_set: Vec<&str> = if labels.is_empty() {
        owned_labels = build_label_set(hypergraph, active_regions);
    ...
    // 6. Coref. Reads `hypergraph.recent_focus` (`hebbian_and_salience`'s output from
    ...
    let coref: Vec<CorefDecision> = resolve_coref(input_text, hypergraph, &ner_spans, policy);
    stage("coref");

    // 7. Surface-OpenIE: SVO + comma-appositive over the orthographic
    //    chunks. Always Defeasible. `build_relations` merges with known-branch
    //    proposals.
...
pub fn run_extractors_simple(input_text: &str, policy: &Policy) -> ExtractionOutput {
    ...
    run_extractors(input_text, &[], policy, &hypergraph, &[])
}
...
fn build_label_set(hypergraph: &Hypergraph, active_regions: &[RegionActivation]) -> Vec<String> {
    let mut seen: std::collections::BTreeSet<String> =
        SEED_KINDS.iter().map(|s| s.to_string()).collect();
    for ra in active_regions {
        let id = ra.region.0 as usize;
        if let Some(el) = hypergraph.elements.get(id)
...
mod tests {
    ...
    fn empty_hypergraph() -> Hypergraph {
        Hypergraph::default()
    }
    ...
    fn run_extractors_finds_dentist_entities() {
        ...
        let hypergraph = empty_hypergraph();
    ...
    fn temporal_pass_emits_weekday_when_ner_misses_it() {
        ...
        let out = run_extractors(text, &["person", "event"], &policy, &hypergraph, &[]);
