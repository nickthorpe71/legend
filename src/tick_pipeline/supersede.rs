//! `supersede` — supersession and cache.
//!
//! Reads `build_relations`'s `minted_relations`, filters to event-shaped
//! relations (have both `from` AND `to` attributes), and for each
//! event:
//!
//! 1. Infers a `property_kind` label from the `from`/`to` value
//!    types via `infer_property_kind` (`new_foundation.md`).
...
//!
//! No model. Everything reads from the seven `build_relations` indices.
...
pub struct Supersession {
    ...
    /// New attribute-name elements minted by `supersede` itself
    ...
    /// Per-tick uncertainty signals raised by `supersede`. `Contradiction`
...
/// Run `supersede` over the relations `build_relations` minted this tick.
///
...
pub fn supersede(
    ...
    let mut out = Supersession::default();

    let Some((from_attr, to_attr)) = signal_from_to_attrs(hypergraph) else {
        return out;
    };
    let intervened_attr = signal_attr(hypergraph, "intervened");

    // Take a snapshot of the relation IDs we care about up front —
    // we'll mutate `hypergraph.relations` (via mint_relation) inside the
    // loop, so we can't hold borrows across iterations.
    let event_ids: Vec<RelationId> = new_relations
        .iter()
        .copied()
        .filter(|&rid| is_event_shaped(hypergraph, rid, from_attr, to_attr))
        .collect();

    for event_id in event_ids {
        let frame = match extract_event_frame(hypergraph, event_id, from_attr, to_attr) {
            Some(f) => f,
            None => continue,
        };

        // Prefer the property slot `build_relations` wrote on the event; fall
        // back to per-value inference for defensiveness (events
        // synthesized outside `build_relations`'s n-ary path may not carry one).
        let property_kind: String = frame
            .property
            .and_then(|pid| hypergraph.elements[pid.0 as usize].names.first().cloned())
            .unwrap_or_else(|| {
                infer_property_kind(hypergraph, frame.from_value, frame.to_value).to_string()
            });
        let cache_attr_label = format!("current_{property_kind}");
        let cache_attr_id = resolve_or_mint_signal_attr(hypergraph, &cache_attr_label, &mut out);
        ...
        let event_conf = hypergraph.relations[event_id.0 as usize].stats.confidence;
        let intervened =
            intervened_attr.is_some_and(|attr| event_has_meta_attr(hypergraph, event_id, attr));
        let gate_passed = intervened || event_conf >= policy.supersession_threshold;

        // ── Prior-cache lookup ─────────────────────────────────────
        // Live priors get flipped only if the gate passed.
        let priors = collect_prior_caches(hypergraph, frame.target, cache_attr_id);
        ...
        if !gate_passed
            && priors.iter().any(|&pid| {
                cache_value_for_attr(hypergraph, pid, cache_attr_id).is_some_and(|v| v != frame.to_value)
            })
        {
            out.uncertainty
        ...
        if std::env::var("LEGEND_DEBUG_STEP9").is_ok() {
            let target_name = hypergraph.elements[frame.target.0 as usize]
                .names
                .first()
                .cloned()
                .unwrap_or_default();
            let to_name = hypergraph.elements[frame.to_value.0 as usize]
                .names
                .first()
                .cloned()
                .unwrap_or_default();
            eprintln!(
                "[supersede] event={} target={target_name:?} cache_attr={cache_attr_label:?} \
        ...
        if gate_passed {
            for &prior in &priors {
                hypergraph.relations[prior.0 as usize].status = RelationStatus::Superseded;
        ...
        if let Some(derived_attr) = signal_attr(hypergraph, "derived_from") {
        ...
        if gate_passed
            && !priors.is_empty()
            && let Some(supersedes_attr) = signal_attr(hypergraph, "supersedes")
        {
            for &prior in &priors {
                let meta = mint_relation(
                    hypergraph,
...
fn event_has_meta_attr(hypergraph: &Hypergraph, event_id: RelationId, attr_name: ElementId) -> bool {
...
fn collect_prior_caches(
    ...
    let Some(candidates) = hypergraph.relations_by_element.get(&target) else {
        return Vec::new();
    };
...
fn cache_value_for_attr(
    ...
    let r = &hypergraph.relations[cache_rid.0 as usize];
    for attr in &r.attributes {
        if attr.name == cache_attr
            && let Term::Element(e) = attr.value
        {
...
/// Slots extracted from an event-shaped Relation. `supersede` needs all
...
/// `build_relations` wrote one; `None` triggers Step-9-side inference.
...
fn signal_from_to_attrs(hypergraph: &Hypergraph) -> Option<(ElementId, ElementId)> {
    let from = signal_attr(hypergraph, "from")?;
    let to = signal_attr(hypergraph, "to")?;
    Some((from, to))
}

fn signal_attr(hypergraph: &Hypergraph, name: &str) -> Option<ElementId> {
    let ids = hypergraph.by_name.get(name)?;
    ids.iter()
        .copied()
...
/// from / to are missing — `build_relations`'s n-ary events should always
/// have all three, but the filter is permissive on shape so guard
/// at extraction time. `property` is optional: `build_relations` began
/// writing it post-Step-9-launch, but pre-existing event relations
/// (from older code paths) may not carry one — `supersede` falls back
...
fn extract_event_frame(
    ...
    let r: &Relation = &hypergraph.relations[rid.0 as usize];
    let target_attr = hypergraph.target_attr;
    let property_attr = hypergraph.by_name.get("property").and_then(|v| v.first().copied());
    let mut target = None;
...
mod tests {
    ...
    fn synth_hypergraph() -> Hypergraph {
        ...
        hypergraph.elements.push(subject);
        hypergraph.elements.push(instance_of);
        hypergraph.subject_attr = ElementId(0);
        hypergraph.by_name.insert("subject".to_string(), vec![ElementId(0)]);
        hypergraph.by_name
    ...
    fn mint_with_kind(hypergraph: &mut Hypergraph, name: &str, kind: &str) -> ElementId {
        ...
        let kind_id = match hypergraph.by_name.get(kind).and_then(|ids| ids.first().copied()) {
            Some(id) => id,
            None => mint_element(
                hypergraph,
                vec![kind.to_string()],
    ...
    fn kind_of_returns_instance_of_kind() {
        let mut hypergraph = synth_hypergraph();
        let tuesday = mint_with_kind(&mut hypergraph, "Tuesday", "weekday");
        assert_eq!(kind_of(&hypergraph, tuesday).as_deref(), Some("weekday"));
    }
    ...
    fn kind_of_returns_none_for_unkinded_value() {
        ...
        assert!(kind_of(&hypergraph, mystery).is_none());
    }

    #[test]
    fn weekday_to_weekday_yields_date() {
        let mut hypergraph = synth_hypergraph();
        let tue = mint_with_kind(&mut hypergraph, "Tuesday", "weekday");
        let fri = mint_with_kind(&mut hypergraph, "Friday", "weekday");
        assert_eq!(infer_property_kind(&hypergraph, tue, fri), "date");
    }

    #[test]
    fn month_to_weekday_still_yields_date() {
        let mut hypergraph = synth_hypergraph();
        let mar = mint_with_kind(&mut hypergraph, "March", "month");
        let fri = mint_with_kind(&mut hypergraph, "Friday", "weekday");
        assert_eq!(infer_property_kind(&hypergraph, mar, fri), "date");
    }

    #[test]
    fn quantity_to_quantity_yields_amount() {
        let mut hypergraph = synth_hypergraph();
        let q1 = mint_with_kind(&mut hypergraph, "$10", "quantity");
        let q2 = mint_with_kind(&mut hypergraph, "$20", "quantity");
        assert_eq!(infer_property_kind(&hypergraph, q1, q2), "amount");
    }

    #[test]
    fn place_to_place_yields_location() {
        let mut hypergraph = synth_hypergraph();
        let a = mint_with_kind(&mut hypergraph, "Berlin", "place");
        let b = mint_with_kind(&mut hypergraph, "Paris", "place");
        assert_eq!(infer_property_kind(&hypergraph, a, b), "location");
    }
    ...
    fn mixed_kinds_fall_back_to_value() {
        let mut hypergraph = synth_hypergraph();
        let tue = mint_with_kind(&mut hypergraph, "Tuesday", "weekday");
        let ten = mint_with_kind(&mut hypergraph, "$10", "quantity");
        assert_eq!(infer_property_kind(&hypergraph, tue, ten), "value");
    }

    /// End-to-end fixture: load the real seed pack, run `run_extractors` and
    /// `build_relations`, then return the Hypergraph + MintedRelations so `supersede`
    /// tests can hand its `minted_relations` to `supersede`. This
    /// integration-style fixture mirrors what `lib.rs::run` does
    /// between `build_relations` and `supersede`.
    fn run_through_build(text: &str, labels: &[&str]) -> (Hypergraph, Vec<RelationId>) {
        ...
        (hypergraph, built_relations.minted_relations)
    }

    #[test]
    fn supersede_skips_non_event_relations() {
        // "Sarah called me yesterday." — produces instance_of
        // relations but no event-shaped from/to relations. `supersede`
        // should mint zero caches.
        let (mut hypergraph, minted) = run_through_build("Sarah called me yesterday.", &[]);
        let policy = crate::types::Policy::default();
        let out = supersede(&mut hypergraph, &minted, &policy);
        assert!(
    ...
    fn supersede_mints_cache_for_dentist_event() {
        let (mut hypergraph, minted) = run_through_build(
        ...
        let current_date_id = hypergraph.by_name["current_date"]
            .iter()
            .copied()
            .find(|id| hypergraph.elements[id.0 as usize].polarity == Polarity::Signal)
            .expect("current_date should have been minted by `supersede`");
        ...
        for &rid in &out.cache_relations {
            ...
            let has_subject = r.attributes.iter().any(|a| a.name == hypergraph.subject_attr);
    ...
    fn prior_cache_flips_to_superseded() {
        ...
        let (mut hypergraph, minted1) = run_through_build(text1, &["event", "weekday"]);
        let policy = crate::types::Policy::default();
        let out1 = supersede(&mut hypergraph, &minted1, &policy);
        assert!(
        ...
        let built_relations =
            crate::tick_pipeline::build_relations::build_relations(text2, &mut hypergraph, &ext2, &policy, None);
        let out2 = supersede(&mut hypergraph, &built_relations.minted_relations, &policy);
        ...
        for &flipped in &out2.superseded {
            assert_eq!(
                hypergraph.relations[flipped.0 as usize].status,
    ...
    fn supersession_emits_derived_from_and_supersedes_metas() {
        ...
        let (mut hypergraph, minted1) = run_through_build(text1, &["event", "weekday"]);
        let policy = crate::types::Policy::default();
        let _ = supersede(&mut hypergraph, &minted1, &policy);
        ...
        let derived_from_id = hypergraph.by_name["derived_from"][0];
        let supersedes_id = hypergraph.by_name["supersedes"][0];
        ...
        for &mid in derived_metas.iter().chain(supersedes_metas.iter()) {
            let r = &hypergraph.relations[mid.0 as usize];
            let target_slot = r
                .attributes
                .iter()
                .find(|a| a.name == hypergraph.target_attr)
    ...
    fn no_prior_no_supersedes() {
        ...
        let (mut hypergraph, minted) = run_through_build(text, &["person", "event", "weekday"]);
        let policy = crate::types::Policy::default();
        let out = supersede(&mut hypergraph, &minted, &policy);
    ...
    fn intervened_event_supersedes_low_confidence_prior() {
        // Use `rescheduled` (in the intervention lexicon) so `build_relations`
        // emits an `intervened` meta. `supersede`'s gate should flip
        ...
        let (mut hypergraph, minted1) = {
            use crate::seed::load_seed_graph;
            use crate::tick_pipeline::build_relations::build_relations;
            use crate::tick_pipeline::run_extractors::run_extractors;
            let mut h = load_seed_graph();
            let ext = run_extractors(text1, &["person", "event", "weekday"], &policy, &h, &[]);
            let built_relations = build_relations(text1, &mut h, &ext, &policy, None);
            (h, built_relations.minted_relations)
        };
        let _ = supersede(&mut hypergraph, &minted1, &policy);
    ...
    fn observed_low_confidence_event_skips_supersession() {
        ...
        let derived_id = hypergraph.by_name["derived_from"][0];
    ...
    fn current_property_attribute_is_reused_across_events() {
        ...
        // Element. Tests `supersede`'s mint-and-cache behavior.
        let (mut hypergraph, minted) = run_through_build(
        ...
        assert!(
            hypergraph.by_name.contains_key("current_date"),
            "`supersede` should have minted current_date",
        );
    }
    ...
    fn two_tick_supersession_integration() {
        ...
        let step9_1 = supersede(&mut hypergraph, &built_relations_1.minted_relations, &policy);
        assert!(
        ...
        let prior_cache: RelationId = step9_1
            ...
            .iter()
            .copied()
            .find(|rid| hypergraph.relations[rid.0 as usize].status == RelationStatus::Asserted)
        ...
        let step9_2 = supersede(&mut hypergraph, &built_relations_2.minted_relations, &policy);

        // (a) Prior flipped.
        assert_eq!(
            hypergraph.relations[prior_cache.0 as usize].status,
            RelationStatus::Superseded,
            "tick 1's cache should now be Superseded",
        );
        assert!(
            step9_2.superseded.contains(&prior_cache),
            "Supersession.superseded should include the flipped prior",
        );
        ...
        let new_r = &hypergraph.relations[new_cache.0 as usize];
        let friday_id = hypergraph.by_name["Friday"][0];
        assert!(
        ...
        let supersedes_attr = hypergraph.by_name["supersedes"][0];
        let target_attr = hypergraph.target_attr;
        let metas_on_new = hypergraph
        ...
        let derived_attr = hypergraph.by_name["derived_from"][0];
        let event_id = built_relations_2
            .minted_relations
            .iter()
            .copied()
            .find(|&rid| {
                let r = &hypergraph.relations[rid.0 as usize];
                let from_attr_id = hypergraph.by_name["from"]
                    .iter()
                    .find(|id| hypergraph.elements[id.0 as usize].polarity == Polarity::Signal)
                    .copied()
                    .unwrap();
                let to_attr_id = hypergraph.by_name["to"]
                    .iter()
                    .find(|id| hypergraph.elements[id.0 as usize].polarity == Polarity::Signal)
                    .copied()
                    .unwrap();
        ...
        let metas_on_old = hypergraph
            .meta_relations_by_object
            .get(&prior_cache)
            .cloned()
            .unwrap_or_default();
        assert!(
            metas_on_old.iter().any(|&mid| hypergraph.relations[mid.0 as usize]
                .attributes
                .iter()
    ...
    fn no_kind_at_all_falls_back_to_value() {
        ...
        assert_eq!(infer_property_kind(&hypergraph, a, b), "value");
    }
}
