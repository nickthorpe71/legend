...
fn discover_reports_repo_shape_without_modifying_memory() {
    ...
    assert!(report["metadata"]["tech_stack"]
        .as_array()
        .unwrap_or(&vec![])
        .iter()
        .any(|item| item == "Rust"));
    assert!(report["high_signal_files"]
        .as_array()
        .unwrap_or(&vec![])
        .iter()
...
fn discover_apply_ingests_context_into_memory() {
    ...
    assert!(short_term.iter().any(|entry| entry["text"]
        .as_str()
        .unwrap_or("")
        .contains("ONBOARDING: Discovery report for fixture-app")));
    assert!(short_term.iter().any(|entry| entry["text"]
        .as_str()
        .unwrap_or("")
        .contains("CONTEXT: High-signal file 'README.md'")));
    assert!(short_term.iter().any(|entry| entry["text"]
        .as_str()
        .unwrap_or("")
        .contains("ONBOARDING TASKS:")));
}
...
fn discover_includes_entry_points_in_high_signal() {
    ...
    assert!(
        high_signal.iter().any(|f| f["path"] == "Cargo.toml"),
