...
- **2026-04-10** — ARCHITECTURE: Improved L2 retrieval with two major changes. (1) Sentence-level chunking in entorhinal.rs — replaced line-accumulation chunker (CHUNK_TARGET_LEN=200) with sentence-boundary splitting.…
- **2026-04-10** — ARCHITECTURE: Major L2 retrieval overhaul — removed three systems that were hiding episodic facts from query results. (1) Removed consolidated filter from top_k_similar and pattern_complete in hippo…
- **2026-04-11** — ARCHITECTURE: LongMemEval full 500-question benchmark results — 70.4% overall (352/500). Breakdown: single-session-user 91.4%, single-session-assistant 87.5%, knowledge-update 84.6%, multi-session 7…
- **2026-04-11** — ARCHITECTURE: LongMemEval temporal-reasoning pipeline analysis. Data flow: (1) LongMemEval oracle.json contains 500 questions with haystack_sessions (conversation turns) and haystack_dates (ISO timest…
- **2026-04-11** — ARCHITECTURE: Implemented full temporal memory system — three temporal signals per memory. (1) Date extraction in wernicke/extract.rs: extract_dates() handles ISO dates (2023/01/15, 2023-01-15), mon…
