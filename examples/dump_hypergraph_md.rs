//! Generate a markdown snapshot of the loaded Hypergraph.
//!
//! Run: `cargo run --release --example dump_hypergraph_md`
//!
//! Output: `inspect/seed.md` (gitignored).
//!
//! Why this exists: at v0 scale (~72 elements, ~53 relations) a
//! markdown report is the fastest way to eyeball "what's in the graph?"
//! after regenerating the seed bin or, later, after a tick mutates
//! the substrate. Renders inline on GitHub via mermaid; diffs cleanly
//! between snapshots.
//!
//! This is *not* a replacement for an interactive viewer. Mermaid
//! breaks down past ~200 nodes; once mutated graphs grow beyond seed
//! scale, switch to the HTML viewer (planned).
//!
//! The function `dump_markdown(hg, path)` takes any `&Hypergraph`, so
//! this generator stays useful when save/load lands — call it against
//! a deserialized snapshot to get the same report shape.

use legend::seed::load_seed_graph;
use legend::types::{ElementId, Hypergraph, RelationStatus, Term};
...
/// Render the full markdown report for `hg`. Pure string assembly — no
/// I/O. Callable from anywhere with a `&Hypergraph`.
...
fn categorize(hg: &Hypergraph) -> Categorized {
    ...
    // Anchors and classes first — explicit IDs cached on the Hypergraph.
    ...
    // Regions — anything appearing as the subject of `parent_region`
    // (i.e. has a parent in the region DAG). Equivalent to the keys of
    // `region_parents`.
    for region in hg.region_parents.keys() {
        by_element.insert(*region, Category::Region);
    }

    // Prototypes — anything appearing as a value of a `prototype`-attr
    // relation (the targets in `region_prototypes`).
    ...
    // Frames — anything appearing as the subject of an `instance_of`
    // relation pointing at `reference_frame_class`. Iterate relations
    // because there's no dedicated index.
    ...
    for r in &hg.relations {
        ...
        if let (Some(s), true) = (subj, points_at_frame_class) {
            // Don't overwrite an existing Class/Anchor categorization.
            by_element.entry(s).or_insert(Category::Frame);
        }
    }

    // Attribute names — two signals, either is sufficient:
    //   (a) the element's ID appears as the `name` slot of any
    //       attribute on any relation (it's been *used* as a slot name);
    //   (b) the element's canonical name matches one of the 30
    //       hardcoded seed-pack attribute-name surface forms (it's
    //       *recognized* as a slot name even if no relation uses it
    //       yet — e.g. `valid_from`, `target`, etc. ship seeded but
    //       won't appear in relations until extractor output lands).
    ...
    // Anything still un-tagged is a tick-time mint (post-v0 substrate).
    for e in &hg.elements {
...
fn write_header(s: &mut String, hg: &Hypergraph, cat: &Categorized) {
    ...
    // Per-attribute-name relation counts.
...
fn write_dag_diagram(s: &mut String, hg: &Hypergraph, cat: &Categorized) {
    ...
    // Declare GENESIS first with a circle shape so it's visually
    // distinct. Subsequent edges reuse the same mermaid id.
    let _ = writeln!(s, "  {}((GENESIS))", mermaid_id(hg, hg.genesis));

    // Region children of any node — walk from GENESIS down. The seed DAG
    // is one level deep, but write a generic walk so it keeps working
    // when the DAG grows.
    let mut visited: HashSet<ElementId> = HashSet::new();
    let mut stack = vec![hg.genesis];
    // Sort for deterministic output order — HashSet/HashMap iteration
    // is otherwise nondeterministic and would churn the diff every run.
    ...
    // Apply visual classes — regions blue, prototypes green, anchors gold.
...
fn write_attribute_names(s: &mut String, hg: &Hypergraph, cat: &Categorized) {
    ...
    // Collect attribute-name elements grouped by hardcoded category.
...
fn write_prototypes(s: &mut String, hg: &Hypergraph, cat: &Categorized) {
    ...
    // Reverse-index proto → region so we can show ownership in the table.
...
fn write_relations_grouped(s: &mut String, hg: &Hypergraph) {
    ...
    // Group: attr_name_id → Vec<(subject_id, object_id, status, conf)>
    ...
    // Sort group keys by canonical name for stable output.
